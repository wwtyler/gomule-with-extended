...
public class D2Prop {
    ...
    public String generateDisplay(int qFlag, int cLvl) {
        ...
        // FUNCTION 0 means that you should use the txt files to find the print function
        // to use. Otherwise, it should be a case of looking for custom funcs
        if (funcN == 0) {
        ...
        switch (funcN) {
            ...
            case (13) -> {
                return "+" + pVals[1] + " to " + D2TxtFile.getCharacterCode(pVals[0])
                        + " Skill Levels";
            }
            ...
            case (19), (29) -> {
                ...
                if (matchingPropsRecords.isEmpty()) {
                    // 检查是否前两个字符是"ÿc"且倒数第三和倒数第二字符是"ÿc"
                    if (oString != null && oString.length() > 6 && oString.startsWith("ÿc")
                            && oString.charAt(oString.length() - 3) == 'ÿ'
                            && oString.charAt(oString.length() - 2) == 'c') {
                        // 去掉前三个字符和后三个字符
                        oString = oString.substring(3, oString.length() - 3);
                    }
                    switch (oString) {
                        case "Indestructible" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "indestruct"));
                        case "Indestructible(Indestructible)" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "indestruct"));
                        case "%+d%% Enhanced Maximum Damage" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "dmg%"));
                        case "%+d to Maximum Damage" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "dmg-max"));
                        case "%+d to Minimum Damage" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "dmg-min"));
                        case "%+d to Maximum Damage(Max) ÿc2" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "dmg-max"));
                        case "%+d to Minimum Damage(Min) ÿc2" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "dmg-min"));
                        case "%+d to Fire Skills" -> matchingPropsRecords =
                                singletonList(D2TxtFile.PROPS.searchColumns("code", "fireskill"));
            ...
            case (23) -> {
                return pVals[1] + "% " + oString + " " + D2Files.getInstance().getTranslations()
            ...
            case (27) -> {
                D2TxtFileItemProperties skillDescRow = D2TxtFile.SKILL_DESC.getRow(pVals[0]);
                String skillName = D2Files.getInstance().getTranslations()
                        .getTranslation(D2TxtFile.SKILL_DESC
                                .searchColumns("skilldesc",
                                        D2TxtFile.SKILLS.getRow(pVals[0]).get("skilldesc"))
                                .get("str name"));
                String charClass =
                        D2TxtFile.SKILLS.getRow(skillDescRow.getRowNum()).get("charclass");
                String classOnly = D2Files.getInstance().getTranslations().getTranslation(
                        charClass.substring(0, 1).toUpperCase() + charClass.substring(1) + "Only");
                return "+" + pVals[1] + " to " + skillName + " " + classOnly;
            }

            case (28) -> {
                String pText = D2TxtFile.SKILLS.getRow(pVals[0]).get("skilldesc");
                String key = D2TxtFile.SKILL_DESC.searchColumns("skilldesc", pText).get("str name");
                return "+" + pVals[1] + " to "
                        + D2Files.getInstance().getTranslations().getTranslation(key);
            }
            ...
            case (35) -> {
                if (pVals[0] == pVals[1]) {
                    return "Adds " + pVals[0] + " Cold Damage Over " + Math.round((double) pVals[2] / 25.0) + " Secs ("
                            + pVals[2] + " Frames)";
                }
            ...
            case (36) -> {
                if (pVals.length == 4) {
                    ...
                    if (pVals[0] == pVals[1]) {
                        return "Adds "
                                + Math.round(
                                        pVals[0] * ((double) pVals[2] / (double) pVals[3]) / 256)
                                + " Poison Damage Over "
                                + (int) Math.floor(((double) pVals[2] / (double) pVals[3]) / 25.0)
                                + " Secs (" + pVals[2] + " Frames)";
                    }

                    return "Adds "
                            + Math.round(pVals[0] * ((double) pVals[2] / (double) pVals[3]) / 256)
                            + " - "
                            + Math.round(pVals[1] * ((double) pVals[2] / (double) pVals[3]) / 256)
                            + " Poison Damage Over "
                            + (int) Math.floor(((double) pVals[2] / (double) pVals[3]) / 25.0)
                            + " Secs (" + pVals[2] + " Frames)";
                    ...
                    if (pVals[0] == pVals[1]) {
                        return "Adds " + Math.round(pVals[0] * (double) pVals[2] / 256)
                                + " Poison Damage Over "
                                + (int) Math.floor((double) pVals[2] / 25.0) + " Secs (" + pVals[2]
                                + " Frames)";
                    }

                    return "Adds " + Math.round(pVals[0] * (double) pVals[2] / 256) + " - "
                            + Math.round(pVals[1] * (double) pVals[2] / 256)
                            + " Poison Damage Over " + (int) Math.floor((double) pVals[2] / 25.0)
                            + " Secs (" + pVals[2] + " Frames)";
                }
            }
    ...
    public void addPVals(int[] newVals) {
        ...
        // Poison length needs to keep track of the number of properties contributing to
        // it.
    ...
    public void addCharMods(int[] outStats, ArrayList<D2Prop> plSkill, int cLvl, int op, int qFlagMarker) {
        ...
        /**
         * 0 Str 0 1 Str/lvl 220 2 En 1 3 En/lvl 222 4 Dex 2 5 Dex/lvl 221 6 Vit
         * 3 7 Vit/lvl 223 8 Life 7 9 HP/lvl 216 10 Mana 9 11 Mana/Lvl 217 12
         * Stam 11 13 Stam/lvl 242 14 AR 19 15 AR/% 119 16 AR/lvl 224 17
         * AR/%/Lvl 225 18 FR 39 19 LR 41 20 CR 43 21 PR 45 22 MF 80 23 MF/Lvl
         * 240 24 FRW 96 25 FCR 105 26 IAS 93 27 FHR 99 28 GF 79 29 GF/lvl 239
         * 30 AllSkill 127 31 Block 20
         */
        switch (pNum) {
